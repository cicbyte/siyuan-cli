...
// CloseOptions 定义notebook close命令的选项
type CloseOptions struct {
	NotebookIdentifier string // 笔记本ID或名称
}

// CloseNotebook 执行关闭笔记本的逻辑
func CloseNotebook(opts CloseOptions) error {
	logger := log.GetLogger()
	logger.Info("开始关闭笔记本", zap.String("identifier", opts.NotebookIdentifier))
	...
	if opts.NotebookIdentifier == "" {
		...
		fmt.Println("💡 使用方法: siyuan-cli notebook close <笔记本名称或ID>")
	...
	for _, nb := range notebooks {
		if nb.ID == targetID && nb.Closed {
			logger.Info("笔记本已经关闭", zap.String("notebook_id", targetID), zap.String("notebook_name", targetName))
			fmt.Printf("📁 笔记本 '%s' 已经关闭\n", targetName)
			return nil
		}
	}

	// 调用关闭笔记本API
	logger.Info("开始调用CloseNotebook API", zap.String("notebook_id", targetID), zap.String("notebook_name", targetName))
	err = client.CloseNotebook(ctx, targetID)
	if err != nil {
		logger.Error("关闭笔记本失败", zap.String("error", err.Error()), zap.String("notebook_id", targetID))

		if syErr, ok := siyuan.IsAPIError(err); ok {
			fmt.Printf("❌ 思源笔记API错误 (code=%d): %s\n", syErr.Code, syErr.Msg)
		} else {
			fmt.Printf("❌ 关闭笔记本失败: %v\n", err)
			// 提供详细的错误诊断
			fmt.Println("\n🔍 错误诊断:")
	...
	logger.Info("成功关闭笔记本", zap.String("notebook_id", targetID), zap.String("notebook_name", targetName))
	fmt.Printf("✅ 成功关闭笔记本: %s (%s)\n", targetName, targetID)
	return nil
}
