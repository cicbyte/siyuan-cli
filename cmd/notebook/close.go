...
func getCloseCommand() *cobra.Command {
	return &cobra.Command{
		Use:   "close [notebook-name-or-id]",
		Short: "关闭笔记本",
		Long: `关闭指定的思源笔记笔记本，支持名称或ID。

参数：
- notebook-name-or-id: 要关闭的笔记本名称或ID（必需）
...
示例：
- siyuan-cli notebook close ai                    # 使用名称
- siyuan-cli notebook close 20251105164527-ezlspgg  # 使用ID
- siyuan-cli notebook list  # 查看所有笔记本`,
		Run: func(cmd *cobra.Command, args []string) {
			...
			opts := notebook.CloseOptions{
				NotebookIdentifier: args[0],
			}

			if err := notebook.CloseNotebook(opts); err != nil {
				fmt.Printf("❌ 命令执行失败: %v\n", err)
				return
			}
		},
